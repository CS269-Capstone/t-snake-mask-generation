...
if __name__ == '__main__':
    ...
    #This is the very shady version of testing the snake

    #initialize snake (rectangle)
    top = []
    bottom = []
    left = []
    right = []
    for i in range(15):
        top.append(Node(50,50 + 20*i))
    for i in range (15):
        right.append(Node(50+20*i, 350))
    for i in range (15):
        bottom.append(Node(350, 350 - 20*i))
    for i in range (15):
        left.append(Node(350 - 20*i, 50))
    ...
    ############ PARAMETERS
    sigma = 20    #gaussian filter sigma
    p = 1         #scale final image force with p
    c = 20        #scale gradient magnitude of image (applied before p)
    a = 1         #tension parameter
    b = 1         #bending parameter
    ...
    #save to csv file for MATLAB (I would just plot in python, easier)
    np.savetxt("mags.csv", mags, delimiter=",")

    #set inflation force to 0 at every pixel just for now
    intensity = np.zeros((400,400,2))

    #initialize snake
    snake = TSnake(top,out,intensity,a,b,gamma,dt)

    M = 20 #number of m-steps (iterations)
    
    #X and Y are matrices to save the position of the snake for every iteration
    X = np.zeros((snake.num_nodes,M+1))
    Y = np.zeros((snake.num_nodes,M+1))
    #print(type(snake.nodes))
    #print(type(snake.nodes[1]))
    ...
    #save X and Y into csv files for MATLAB (just plot in python...)
